...
def plummer_w_binaries(N, f_bin=1.,logamin=-4,logamax=-2,seed=123454321):
  random.seed(seed)
...
def timerun(N=1000,f_bin=1.,tend=1.| nbody_system.time,inttype=Huayno.inttypes.HOLD_DKD,seed=12345678,logamin=-4):
  ...
  for inttype in inttypes:
    ...
    for N in Ns:
      ...
      for i in range(Nsample):
        seed=123456+i
        t,de,dp=timerun(N=N,f_bin=1.,inttype=inttype,seed=seed,logamin=logamin)
        print t,de,dp.length()
        data[inttype][N].append( (t,de,dp) )
    print     
...
def doplot(label=""):
  ...
  for inttype in inttypes:
    ...
    maxerr=[]
    for N in Ns:
      t=numpy.array(map(lambda x: x[0],data[inttype][N]))
      de=numpy.array(map(lambda x: abs(x[1]),data[inttype][N]))
  ...
  for i,inttype in enumerate(inttypes):
    ...
    ax.loglog(Ns,eerr,'r'+linestyles[i])
...
if __name__=="__main__":
  generate_data(Nsample=3,logamin=-3,label="-3")
