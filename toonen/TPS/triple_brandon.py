...
class Triple_Class:
    ...
    def __init__(self, inner_primary_mass, inner_secondary_mass, outer_mass,
            ...
            inner_mean_anomaly, outer_mean_anomaly,
...
#            exit(1)
    ...
    def make_bins(self, stars, inner_semimajor_axis, outer_semimajor_axis,
        ...
        bins[0].mean_anomaly = inner_mean_anomaly
        ...
        bins[1].mean_anomaly = outer_mean_anomaly
...
#            print 'freq', stellar_system.spin_angular_frequency, corotating_spin_angular_frequency_binary(stellar_system.parent.semimajor_axis, self.get_mass(stellar_system.parent.child1), self.get_mass(stellar_system.parent.child2))
        else:
    ...
    def update_time_derivative_of_radius(self, stellar_system = None):
        ...
        if self.time == quantities.zero:
            ...
            if stellar_system.is_star:
                if REPORT_DEBUG:
                    print 'times', self.time, self.previous_time, time_step
                    print 'R dot', stellar_system.radius, stellar_system.previous_radius, time_step,  (stellar_system.radius - stellar_system.previous_radius)/time_step
                    print 'R dot/R', (stellar_system.radius - stellar_system.previous_radius)/time_step/stellar_system.radius
    ...
    def update_time_derivative_of_moment_of_inertia(self, stellar_system = None):
        ...
        if self.time == quantities.zero:
            ...
            if stellar_system.is_star:
                if REPORT_DEBUG:
                    print 'moi dot', stellar_system.moment_of_inertia_of_star, stellar_system.previous_moment_of_inertia_of_star, time_step,  (stellar_system.moment_of_inertia_of_star - stellar_system.previous_moment_of_inertia_of_star)/time_step
                    print 'moi dot/moi', (stellar_system.moment_of_inertia_of_star - stellar_system.previous_moment_of_inertia_of_star)/time_step/stellar_system.moment_of_inertia_of_star
...
#            print 'Roche lobe radii:', Rl1, Rl2, Rl3
            Rl1 = roche_radius(bin, bin.child1, self) * (1-bin.eccentricity)
            Rl2 = roche_radius(bin, bin.child2, self) * (1-bin.eccentricity)
            Rl3 = roche_radius(self.triple, star, self)*(1-self.triple.eccentricity)
...
#        time_step_radius_change = self.determine_time_step_radius_change()             
        time_step_radius_change = time_step_max
...
#                print 'donor time:',  self.determine_time_step_stable_mt()
        ...
        if REPORT_DT or REPORT_TRIPLE_EVOLUTION or REPORT_DEBUG:
            print 'time:', time_step_max, time_step_stellar_code, time_step_wind, time_step_radius_change
...
#                    time_step = min(time_step, time_step_close_to_mt)
      ...
      # in case secular code finds RLOF 
        if self.fixed_timestep > 0.|units.yr:
            if REPORT_DT or REPORT_TRIPLE_EVOLUTION or REPORT_DEBUG:
                print 'sec time:', self.fixed_timestep
                print 'donor time:',  self.determine_time_step_stable_mt()
...
#            print previous_time_step, maximum_time_step_factor*previous_time_step, time_step
            if REPORT_DT or REPORT_TRIPLE_EVOLUTION or REPORT_DEBUG:
                print 'min increase', time_step, maximum_time_step_factor*previous_time_step                                
...
#            print time_step_max, time_step_stellar_code, time_step_wind, time_step_radius_change, time_step
    ...
    def adjust_system_after_supernova_kick(self):
        ...
        if which_mean_anomaly == 1: #user specified
            inner_mean_anomaly = self.triple.child2.mean_anomaly
            outer_mean_anomaly = self.triple.mean_anomaly
...
#            print 'eccentricity:', inner_ecc, outer_ecc
    ...
    def solve_for_partial_time_step_stable_mass_transfer(self):
        ...
        self.secular_code.model_time = self.time # not redundant!
...
#                    self.channel_from_stellar.copy()
    ...
    def rewind_to_begin_of_rlof_stellar(self, dt):          
        ...
        while self.has_donor() == True and dt_new > dt_min:
            dt_old = self.time - self.previous_time
            dt_new = max(minimum_time_step, 0.5*dt_old)

            self.stellar_code.particles.recall_memory_one_step()
            self.time += (dt_new - dt_old)   
            if REPORT_TRIPLE_EVOLUTION or REPORT_DEBUG:
    ...
    def rewind_to_begin_of_rlof_secular(self):          
        ...
        if self.fixed_timestep < 0.|units.yr:
            print 'fixed_timestep < 0: should not be possible', self.time, self.secular_code.model_time, self.fixed_timestep
            exit(1)                    
        #rewind system
        self.stellar_code.particles.recall_memory_one_step()
        self.refresh_memory() 
...
#-------------------------    
    ...
    def evolve_model(self):
        ...
        self.save_snapshot()  
        self.triple.bin_type = bin_type['detached'] 
        self.triple.child2.bin_type = bin_type['detached'] 
        while self.time<self.tend: 
            print self.time
            print 'star1:', self.triple.child2.child1.stellar_type, self.triple.child2.child1.mass,
            print 'star2:', self.triple.child2.child2.stellar_type, self.triple.child2.child2.mass,
            print 'star3:', self.triple.child1.stellar_type, self.triple.child1.mass
...
def main(inner_primary_mass= 1.3|units.MSun, inner_secondary_mass= 0.5|units.MSun, outer_mass= 0.5|units.MSun,
            ...
            inner_mean_anomaly = 0.0, outer_mean_anomaly = 0.0,
            metallicity= 0.02,
    ...
    inner_mean_anomaly = float(inner_mean_anomaly)
    outer_mean_anomaly = float(outer_mean_anomaly)
...
def parse_arguments():
    ...
    parser.add_option("-M", unit=units.MSun, 
                      dest="inner_primary_mass", type="float", default = 9.|units.MSun,
                      help="inner primary mass [%default]")
    parser.add_option("-m",  unit=units.MSun, 
                      dest="inner_secondary_mass", type="float", default = 8.5|units.MSun,
                      help="inner secondary mass [%default]")
    parser.add_option("-l",  unit=units.MSun, 
                      dest="outer_mass", type="float", default = 1.|units.MSun,
                      help="outer mass [%default]")

    parser.add_option("-A", unit=units.RSun,
                      dest="inner_semimajor_axis", type="float", 
                      default = 1.e4 |units.RSun,
                      help="inner semi major axis [%default]")
    parser.add_option("-a", unit=units.RSun,
                      dest="outer_semimajor_axis", type="float", 
                      default = 1.e6 |units.RSun,
                      help="outer semi major axis [%default]")
    parser.add_option("-E",
                      dest="inner_eccentricity", type="float", default = 0.0,
                      help="inner eccentricity [%default]")
    parser.add_option("-e",
                      dest="outer_eccentricity", type="float", default = 0.0,
                      help="outer eccentricity [%default]")
    parser.add_option("-i","-I",
                      dest="relative_inclination", type="float", default = 0.0*np.pi/180.0,
                      help="relative inclination [rad] [%default]")
    parser.add_option("-G",
                      dest="inner_argument_of_pericenter", type="float", default = 0.0,
                      help="inner argument of pericenter [rad] [%default]")
    parser.add_option("-g",
                      dest="outer_argument_of_pericenter", type="float", default = 0.0,
...
#                      help="outer longitude of ascending node [rad] [%default]")
    parser.add_option("-X",
                      dest="inner_mean_anomaly", type="float", default = 0.0, #at periaps
                      help="inner mean anomaly [rad] [%default]")
    parser.add_option("-x",
                      dest="outer_mean_anomaly", type="float", default = 0.0,#at periaps
                      help="outer mean anomaly [rad] [%default]")
    ...
    parser.add_option("-t", "-T", unit=units.Myr, 
                      dest="tend", type="float", default = 35.0 |units.Myr,
...
if __name__ == '__main__':
    ...
    if triple_class_object.triple.dynamical_instability_at_initialisation == True:
        ...
        if REPORT_TRIPLE_EVOLUTION or REPORT_USER_WARNINGS:
