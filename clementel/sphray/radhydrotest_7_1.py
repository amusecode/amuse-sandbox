...
Tion=13500. | units.K
rhoinit=2.e-4 | (units.amu / units.cm**3)
rhoclump=0.04 | (units.amu / units.cm**3)  
uinit=1/(gamma-1)*constants.kB * Tinit/mu
uclump=1/(gamma-1)*constants.kB * Tclump/mu
...
def iliev_test_7_ic( N=10000,
  ...
  sel=numpy.where(((x-(5./6.6))**2+y**2+z**2)**0.5>(0.8/6.6))[0]
...
def test(evolver, rad_parameters=dict(),dt=1. | units.Myr ):    
  ...
  sph,rad=iliev_test_7(N,Ns,L,dt,rad_parameters=rad_parameters)
  ...
  for tend in tplot:
    ...
    sel=numpy.where(((p.z**2+p.y**2)**0.5)< (0.35 | units.kpc))[0]
    xaxis=p.x[sel].value_in(units.kpc)
    xionaxis=p.xion[sel].value_in(units.none)
    musaxis=mu/(1+xionaxis)
    taxis=(p.u[sel]*musaxis/constants.kB).value_in(units.K)
    rhoaxis=p.rho[sel]
    presaxis=(p.u[sel]*rhoaxis).value_in(units.g/units.cm/units.s**2)

    i=int(tend/dt)

    aplot(i,'xion',((xaxis/6.6,1.-xionaxis,'r'),),
         xlim=(-1.,1.),ylim=(1.e-9,2.))
    aplot(i,'temp',((xaxis/6.6,taxis,'r'),),
        xlim=(-1.,1.),ylim=(4.e3,1.e5))
    aplot(i,'pres',((xaxis/6.6,presaxis,'r'),),
        xlim=(-1.,1.),ylim=(1.e-16,1.e-12))
...
if __name__=="__main__":
  ...
  t2=time.time()
  print "test7: 5 freq, dynamic, thermal solver",
  print t2-t1,"sec"
