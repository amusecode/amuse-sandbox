...
void main() {
	...
	vec4 sphereColor = vec4(texture(sphereTexture, tCoord).rgb, 1.0);
    ...
    vec4 color = mix(starColor * starBrightness, starHaloColor * starHaloBrightness, 0.5); 
    	 color = mix(color, sphereColor * sphereBrightness * 2.5, 0.5);
    	 color = mix(color, gasColor * gasBrightness * 2.5, 0.1);
