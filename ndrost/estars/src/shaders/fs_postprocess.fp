...
void main() {
	...
	vec4 axesColor = vec4(texture(axesTexture, tCoord).rgb, 1.0) * axesBrightness;
  	vec4 pointGasColor  = vec4(texture(pointGasTexture, tCoord).rgb, 1.0) * pointGasBrightness;	
  	vec4 octreeGasColor  = vec4(texture(octreeGasTexture, tCoord).rgb, 1.0) * octreeGasBrightness;	
	vec4 sphereColor = vec4(texture(sphereTexture, tCoord).rgb, 1.0) * sphereBrightness;
	vec4 starColor = vec4(texture(starTexture, tCoord).rgb, 1.0) * starBrightness;
	vec4 starHaloColor = vec4(texture(starHaloTexture, tCoord).rgb, 1.0) * starHaloBrightness;	
	vec4 hudColor = vec4(texture(hudTexture, tCoord).rgb, 1.0) * hudBrightness;
    
    vec4 color = axesColor + pointGasColor + octreeGasColor + sphereColor + starColor + starHaloColor + hudColor;
