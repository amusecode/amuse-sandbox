...
static int kepler_tolerance_level = 2;
...
void kepler::initialize_from_pos_and_vel(bool minimal, bool verbose)
{
    ...
    if (energy != 0) {
        ...
        // different expressions for eccentricity in case of bound or unboud orbit
        if (temp > 1.0) {
          eccentricity = sqrt(1.0 - angular_momentum*angular_momentum/total_mass/semi_major_axis);
        } else {
          eccentricity = sqrt(rdotv * rdotv / (total_mass * semi_major_axis) + temp * temp);
        }

        // to check the difference in expressions for eccenticity
//         real eccentricity_1 = sqrt(1.0 - angular_momentum*angular_momentum/total_mass/semi_major_axis);
//         real eccentricity_2 = sqrt(rdotv * rdotv / (total_mass * semi_major_axis) + temp * temp);
//         real dife = sqrt((eccentricity_1 - eccentricity_2)*(eccentricity_1 - eccentricity_2));
//         if (dife > 10.0e-8) {
//           printf (" e_diff %15.10e %15.10e \n", eccentricity_1, eccentricity_2);
//         }
//         if (energy > 0) {
//                   if (eccentricity < 1.0) {
//                     printf ("ener %15.10e \n", energy);
//                     printf ("ecc %15.10e \n", eccentricity);
//                     printf ("sep %15.10e \n", separation);
//                     printf ("rel_vel %15.10e %15.10e %15.10e\n", rel_vel[0], rel_vel[1], rel_vel[2]);
//                     printf ("rel_pos %15.10e %15.10e %15.10e\n", rel_pos[0], rel_pos[1], rel_pos[2]);
//                     printf ("a %15.10e \n", semi_major_axis);
//                     printf ("temp %15.10e \n", temp);
//                     printf ("rdotv %15.10e \n", rdotv);
//                     printf ("mass %15.10e \n", total_mass);
...
//          }
	...
	if (eccentricity != 1) {
	    ...
	    cos_true_an = (periastron/separation) * (1./eccentricity + 1) - 1/eccentricity;
        }
        ...
        if(fabs(cos_true_an)>1)
        cout << "r,pa,e,cos" << separation <<" "<< periastron << " " << eccentricity
           << " " << cos_true_an << endl;
