...
def multiplesclass(gravity):
    class multiples(gravity):
        def __init__(self, converter, 
                     delta_t = 1 | nbody_system.time,
                     SmallN=SmallN, 
                     **karguments):
            print "kw=", karguments
            gravity.__init__(self,converter, **karguments)
        ...
        def evolve_model(self, end_time):
            ...
            stars = self.particles.copy()
            stars.id = self.particles.index_in_code

            channel = self.particles.new_channel_to(stars)
            stopping_condition = self.stopping_conditions.collision_detection
            stopping_condition.enable()
            E0 = print_log('ph4', self)
...
def test_multiples(infile = None, number_of_stars = 40,
    ...
    multiples=multiplesclass(grav)

    if gpu_worker == 1:
        try:
            gravity = multiples(converte, delta_t, SmallN, number_of_workers = n_workers,
                                redirection = "none", mode = "gpu")
        except Exception as ex:
            gravity = multiples(converter, delta_t, SmallN, number_of_workers = n_workers, redirection = "none")
    else:
        gravity = multiples(converter, delta_t, SmallN, number_of_workers = n_workers, redirection = "none")
