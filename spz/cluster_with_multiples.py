...
class multiples(object):
    def __init__(self, converter, gravity, 
                 delta_t = 1 | nbody_system.time,
                 SmallN=SmallN, 
                 **karguments):
        print "kw=", karguments
        self.gravity = gravity(converter, **karguments)
        self.gravity.initialize_code()
        self.gravity.parameters.set_defaults()
        self.delta_t = delta_t
        self.SmallN = SmallN
        self.dEmult = 0
    @property 
    def particles(self):
        return self.gravity.particles
    @property 
    def total_mass(self):
        return self.gravity.total_mass
    @property 
    def kinetic_energy(self):
        return self.gravity.kinetic_energy
    @property 
    def potential_energy(self):
        return self.gravity.potential_energy
    @property 
    def parameters(self):
        return self.gravity.parameters
    #Add?: def gravity.commit_particles()
    def get_gravity_at_point(self,radius, x, y, z):
        return self.get_gravity_at_point(radius, x, y, z)
    def get_potential_at_point(self, radius, x, y, z):
        return self.get_potential_at_point(radius, x, y, z)
    def commit_particles(self):
        return self.gravity.commit_particles()
    def get_time(self):
        return self.gravity.get_time()
    def evolve_model(self, end_time):
        time = self.gravity.model_time
        print "Evolve model to", end_time, time, self.delta_t
        stars = self.gravity.particles.copy()
        stars.id = self.gravity.particles.index_in_code

        channel = self.gravity.particles.new_channel_to(stars)
        stopping_condition = self.gravity.stopping_conditions.collision_detection
        stopping_condition.enable()
        E0 = print_log('ph4', self.gravity)
        ...
        while time < end_time:
            time += self.delta_t
            while self.gravity.get_time() < time:
                ...
                if stopping_condition.is_set():
                    ...
                    print 'stopping condition set at time', \
                        self.gravity.get_time().number

                    E = print_log('ph4', self.gravity, E0)
                    print 'dEmult =', self.dEmult, 'dE =', (E-E0).number-self.dEmult
                    ...
                    self.gravity.synchronize_model()

                    self.dEmult += manage_encounter(star1, star2, stars,
                    ...
                    # it. TODO
                    self.gravity.recommit_particles()
...
def manage_encounter(star1, star2, stars, gravity_stars, SmallN):
    ...
    run_smallN(particles_in_encounter, SmallN=SmallN)
...
def test_multiples(infile = None, number_of_stars = 40,
    ...
    if gpu_worker == 1:
        try:
            gravity = multiples(converte, grav, delta_t, SmallN, number_of_workers = n_workers,
                                redirection = "none", mode = "gpu")
        except Exception as ex:
            gravity = multiples(converter, grav, delta_t, SmallN, number_of_workers = n_workers, redirection = "none")
    else:
        gravity = multiples(converter, grav, delta_t, SmallN, number_of_workers = n_workers, redirection = "none")
    ...
    E0 = print_log('ph4', gravity.gravity)
    dEmult = 0.0

    gravity.evolve_model(end_time)

    print ''
    gravity.gravity.stop()
