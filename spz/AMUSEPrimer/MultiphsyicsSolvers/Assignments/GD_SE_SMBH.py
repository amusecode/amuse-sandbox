...
def construct_star(mZAMS, z, time):
    star = SeBa()
    star.particles.add_particle(Particle(mass=mZAMS))
    star.parameters.metallicity = z
    star.commit_particles()
    star.evolve_model(time)
...
def main(N=10, W0=7.0, t_end=10, dt=1, filename="nbody.hdf5", Rvir=1, Mmin=0.1, Mmax= 100, z=0.02):
    ...
    ages = 10000*numpy.random.random(len(masses)) | units.Myr
    for mi, ti in zip(masses, ages):
        star = construct_star(mi, z, ti)
    ...
    bh = Particle(mass = 1.e+6|units.MSun)
    bh.radius = 0 | units.RSun
    bh.position = (0,0,0) | units.parsec
    bh.velocity = (0,0,0) | units.kms
    gravity.particles.add_particle(bh)
