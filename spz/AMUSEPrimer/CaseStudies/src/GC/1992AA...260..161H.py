...
def main(Mstar=1, Ndisk=100, Mdisk= 0.001, Rmin=1, Rmax=100, t_end=10, n_steps=10, filename="1992AA...260..161H", Bump=False):
    ...
    Rmin = Rmin | units.parsec
    Rmax = Rmax | units.parsec
...
#                              discfraction=Mdisk/Mstar).result
    disk = new_plummer_gas_model(Ndisk, convert_nbody=converter)
    disk.position = [1, 0, 0] | units.parsec
    disk.velocity = [0, 0, 0] | units.kms
    print disk
    ...
    if Bump==True:
        ...
        print "Nbump=", Nbump
    disk.h_smooth= 0.001 | units.parsec
    star=Particles(1)
    Mstar = 3.5e+6|units.MSun
    star.mass=Mstar
