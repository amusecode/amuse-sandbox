...
def main(Mstar=1, Ndisk=100, Mdisk= 0.01, t_end=1, n_steps=10, filename="nbody.hdf5"):
#    numpy.random.seed(111)
    Mstar = Mstar | units.MSun
    Mdisk = Mdisk | units.MSun
    t_end = t_end | units.hour
...
#    RLOF donor
    ...
    Rmax = 0.7*Roche_wd
    Rmin = 0.1*Rmax
    print "Roche=", Roche_wd, Rmin, Rmax
...
def new_option_parser():
    ...
    result.add_option("-t", dest="t_end", type="float", default = 1.0,
                      help="end time of the simulation [1] hour")
    return result
