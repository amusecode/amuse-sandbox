...
class AarsethZare(InCodeComponentImplementation):
    ...
    def evolve_model(self, tend):
        ...
        if hasattr(self.particles,"time"):
            ...
            time = tend.as_vector_with_length(len(self.particles)).as_zeros()
        print "Time1=", time
        time, x, y, z, vx, vy, vz = self.call_aarseth_zare(time,mass,x,y,z,vx,vy,vz,tend.as_vector_with_length(len(self.particles)))
        print "Time=", self.particles.time, time, tend, tend.as_vector_with_length(len(self.particles))
