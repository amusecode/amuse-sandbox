...
int commit_particles(){
    ...
    for (map<long long, sph_state>::iterator state_iter = sph_states.begin();
        ...
        P[i].Type = 0; // SPH particles (dark matter particles have type 1-5)
    ...
    for (map<long long, dynamics_state>::iterator state_iter = dm_states.begin();
        ...
        P[i].Vel[1] = (*state_iter).second.vy * a;
        P[i].Vel[2] = (*state_iter).second.vz * a;
        P[i].Type = (*state_iter).second.ptype; // set particle type (SPH particles have type 0)
    }
    dm_states.clear();
...
int new_dm_particle(int *id, double mass, double x, double y, double z, double vx, double vy, double vz, int ptype){
    if (ptype < 1 || ptype > 5) // only collisionless particles
        return -1;
    particle_id_counter++;
    if (ThisTask == 0)
...
int get_epsilon_ptype1(double *epsilon){
    if (ThisTask) {return 0;}
    set_softenings();
    *epsilon = All.SofteningTable[1];
    return 0;
}
int set_epsilon_ptype1(double epsilon){
    All.SofteningHalo = epsilon;
    set_softenings();
    return 0;
}
int get_epsilon_ptype2(double *epsilon){
    if (ThisTask) {return 0;}
    set_softenings();
    *epsilon = All.SofteningTable[2];
    return 0;
}
int set_epsilon_ptype2(double epsilon){
    All.SofteningDisk = epsilon;
    set_softenings();
    return 0;
}
int get_epsilon_ptype3(double *epsilon){
    if (ThisTask) {return 0;}
    set_softenings();
    *epsilon = All.SofteningTable[3];
    return 0;
}
int set_epsilon_ptype3(double epsilon){
    All.SofteningBulge = epsilon;
    set_softenings();
    return 0;
}
int get_epsilon_ptype4(double *epsilon){
    if (ThisTask) {return 0;}
    set_softenings();
    *epsilon = All.SofteningTable[4];
    return 0;
}
int set_epsilon_ptype4(double epsilon){
    All.SofteningStars = epsilon;
    set_softenings();
    return 0;
}
int get_epsilon_ptype5(double *epsilon){
    if (ThisTask) {return 0;}
    set_softenings();
    *epsilon = All.SofteningTable[5];
    return 0;
}
int set_epsilon_ptype5(double epsilon){
    All.SofteningBndry = epsilon;
    set_softenings();
    return 0;
}
int get_epsilon(double *epsilon){
    return get_epsilon_ptype1(epsilon);
}
int set_epsilon(double epsilon){
    return set_epsilon_ptype1(epsilon);
}
...
int get_softening_halo_max_phys(double *softening_max_phys){
    if (ThisTask) {return 0;}
    *softening_max_phys = All.SofteningHaloMaxPhys;
    return 0;
}
int set_softening_halo_max_phys(double softening_max_phys){
    All.SofteningHaloMaxPhys = softening_max_phys;
    return 0;
}
int get_softening_disk_max_phys(double *softening_max_phys){
    if (ThisTask) {return 0;}
    *softening_max_phys = All.SofteningDiskMaxPhys;
    return 0;
}
int set_softening_disk_max_phys(double softening_max_phys){
    All.SofteningDiskMaxPhys = softening_max_phys;
    return 0;
}
int get_softening_bulge_max_phys(double *softening_max_phys){
    if (ThisTask) {return 0;}
    *softening_max_phys = All.SofteningBulgeMaxPhys;
    return 0;
}
int set_softening_bulge_max_phys(double softening_max_phys){
    All.SofteningBulgeMaxPhys = softening_max_phys;
    return 0;
}
int get_softening_stars_max_phys(double *softening_max_phys){
    if (ThisTask) {return 0;}
    *softening_max_phys = All.SofteningStarsMaxPhys;
    return 0;
}
int set_softening_stars_max_phys(double softening_max_phys){
    All.SofteningStarsMaxPhys = softening_max_phys;
    return 0;
}
int get_softening_bndry_max_phys(double *softening_max_phys){
    if (ThisTask) {return 0;}
    *softening_max_phys = All.SofteningBndryMaxPhys;
    return 0;
}
int set_softening_bndry_max_phys(double softening_max_phys){
    All.SofteningBndryMaxPhys = softening_max_phys;
    return 0;
}
...
int get_ptype(int *index, int *ptype, int length){
    int errors = 0;
...
int set_ptype(int *index, int *ptype, int length){
    int *count = new int[length];
    int local_index;

    for (int i = 0; i < length; i++){
        if(found_particle(index[i], &local_index)){
            P[local_index].Type = ptype[i];
            count[i] = 1;
        } else count[i] = 0;
    }
...
int get_epsilon_dm_part(int *index, double *epsilon, int length){
    int *ptype = new int[length];
    set_softenings();
    if (ThisTask) {return 0;}
    double a;
    if (All.ComovingIntegrationOn) {a = All.Time;} else {a = 1;}
    get_ptype(index, ptype, length);
    for (int i = 0; i < length; i++){
        epsilon[i] = a * All.SofteningTable[ptype[i]];//FIXME: must be ptype
    }
    delete[] ptype;
    return 0;
}
