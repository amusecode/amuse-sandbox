...
def new_gravity_code(stars, convert_nbody):
    gravity = ph4(convert_nbody, mode='gpu', channel_type='ibis', hostname='lgm02', 
...
def main():
    ...
    initial_mass_function = new_scalo_mass_distribution # or new_salpeter_mass_distribution or new_flat_mass_distribution

    numpy.random.seed(12345)
    masses = initial_mass_function(number_of_stars, mass_max = 100 | units.MSun).sorted()
    print "Drew {0} random masses from IMF using {1}.\n  maximum={2},\n  mean={3},\n  median={4}".format(
        number_of_stars, initial_mass_function.__name__, masses.amax(), masses.mean(), masses.median())
    
    convert_nbody = nbody_system.nbody_to_si(masses.sum(), 0.31 | units.parsec)
    stars = new_king_model(number_of_stars, 6, convert_nbody)
    stars.mass = masses
