"""
AMUSE Tutorial, step B: n-body dynamics, masses follow Salpeter distribution
"""
import os
import os.path
import shutil
...
def new_working_directory():
    ...
    while os.path.exists(os.path.join(current_directory, "B_run_{0:=03}".format(i))):
        i += 1
    new_directory = os.path.join(current_directory, "B_run_{0:=03}".format(i))
...
if __name__ == "__main__":
    ...
    stars.mass = new_salpeter_mass_distribution(number_of_particles)
    stars.move_to_center()
    stars.scale_to_standard(convert_nbody=unit_converter)
