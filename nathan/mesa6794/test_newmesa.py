...
class TestMESAInterface(TestWithMPI):
    ...
    def test1(self):
        ...
        instance = self.new_instance_of_an_optional_code(MESAInterface, redirection="none")
    ...
    def test3(self):
        ...
        error = instance.set_zamsfile("zams_z2m2_y28.data")
        self.assertEquals(0, error)
        (zamsfile, error) = instance.get_zamsfile()
        self.assertEquals(0, error)
        self.assertEqual(zamsfile, "zams_z2m2_y28.data")
        
        number_of_stars = 2
        for i in range(number_of_stars):
            print i
            (index_of_the_star, error) = instance.new_zams_star(0.5+i*1.0/number_of_stars)
            self.assertEquals(0, error)
    ...
    def test4(self):
        ...
        status = instance.initialize_code()
        error = instance.set_zamsfile("zams_z2m2_y28.data")
        (index_of_the_star, error) = instance.new_zams_star(1.0)
        self.assertEquals(0, error)
        ...
        next_timestep = instance.get_time_step(index_of_the_star).values()[0]
        self.assertTrue(next_timestep <= initial_dt*max_dt_factor)
        self.assertTrue(next_timestep >= initial_dt*min_dt_factor)
    ...
    def slowtest5(self):
        print "Testing evolve with varying Z"
        instance = self.new_instance_of_an_optional_code(MESAInterface)
        instance.set_MESA_paths(instance.default_path_to_inlist, 
            instance.default_path_to_MESA, instance.get_output_directory())
        status = instance.initialize_code()
        ...
        for (i,(Z,L)) in enumerate(zip(metallicities, luminosities)):
            ...
            (index_of_the_star, status) = instance.new_prems_star(1.0)
            self.assertEquals(0, status)
            if (i == 0):
                status = instance.commit_particles()
                self.assertEqual(status,0)
            else:
                status = instance.recommit_particles()
                self.assertEqual(status,0)
            self.assertEqual(index_of_the_star,i+1)
            instance.evolve_for(index_of_the_star, 5.0e-5)
            (L_of_the_star, status) = instance.get_luminosity(index_of_the_star)
            self.assertEquals(0, status)
            print Z, L_of_the_star
#~            self.assertAlmostEqual(L_of_the_star,L,1)
    ...
    def test8(self):
        ...
        for x in ['Reimers', 'Blocker', 'de Jager', 'van Loon', 'Nieuwenhuijzen', 'Kudritzki', 'Vink', 'Dutch']:
            error = instance.set_RGB_wind_scheme(x)
            self.assertEquals(0, error)
            (value, error) = instance.get_RGB_wind_scheme()
            self.assertEquals(0, error)
            self.assertEquals(x, value)
        error = instance.set_RGB_wind_scheme('bogus')
        self.assertEquals(-1, error)
        (value, error) = instance.get_RGB_wind_scheme()
        self.assertEquals(0, error)
        self.assertEquals('Dutch', value)
...
class TestMESA(TestWithMPI):
    ...
    def test4(self):
        ...
        instance.initialize_code() 
        instance.commit_parameters() 
        instance.particles.add_particle(Particle(mass = 1.0 | units.MSun))
        
        initial_dt = 1.0e5 | units.yr
        min_dt_factor = 0.8
        max_dt_factor = 1.2
        self.assertAlmostEqual(instance.particles.time_step, initial_dt)
        instance.particles.evolve_one_step()
        self.assertAlmostEqual(instance.particles.age, initial_dt)
        next_timestep = instance.particles[0].time_step
        self.assertTrue(next_timestep <= initial_dt*max_dt_factor)
        self.assertTrue(next_timestep >= initial_dt*min_dt_factor)
        
        target_end_time = 3.0e5 | units.yr
        instance.particles.evolve_for(target_end_time - instance.particles.age)
        self.assertTrue(instance.particles[0].age >= target_end_time)
        
        L_of_the_star = instance.particles[0].luminosity
        self.assertAlmostEqual(L_of_the_star,0.725 | units.LSun,1)
        M_of_the_star = instance.particles[0].mass
        self.assertAlmostEqual(M_of_the_star,1.000 | units.MSun,3)
        T_of_the_star = instance.particles[0].temperature
    ...
    def test6(self):
        ...
        print instance.particles.get_number_of_zones()
    ...
    def slowtest8(self):
        ...
        instance.particles.add_particles(stars)
        instance.commit_particles()
        instance.evolve_model(5.9 | units.Gyr)
        self.assertTrue(instance.particles[0].age >= 5.9 | units.Gyr)
        ...
        self.assertEquals(number_of_zones,    980)
        self.assertEquals(number_of_species,    8)
        self.assertEquals(len(species_names),  number_of_species)
        self.assertEquals(len(composition),    number_of_species)
        self.assertEquals(len(composition[0]), number_of_zones)
...
#~        self.assertAlmostRelativeEquals(instance.particles.time_step, 1.2*([100000.0, 17677.6695, 1.44*6415.0029] | units.yr), 4)
        ...
        self.assertAlmostRelativeEquals(instance.particles[1:].age, ([17677.6695, 6415.0029*(1+1.2+1.44)] | units.yr)
            + 1.2*([17677.6695, 1.44*6415.0029] | units.yr), 5)
