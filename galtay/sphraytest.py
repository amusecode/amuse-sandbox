...
def read_gas_file(filename):
    ...
    mass=p.mass.number
    h_smooth=p.smoothing_length.number
    x=p.x.number
    y=p.y.number
    z=p.z.number
    rho=p.rho.number
    u=p.internal_energy.number
    xion=numpy.zeros_like(rho)

    pset = create_particle_set( 
      mass=mass | (10**10*units.MSun), 
      h_smooth=h_smooth | (units.kpc), 
      x=x | (units.kpc), 
      y=y | (units.kpc), 
      z=z | (units.kpc), 
      rho=rho | ((10**10*units.MSun) /(units.kpc)**3),
      xion=xion, 
      u=u | (10**5 * units.cm/units.s)**2 )
...
def read_src_file(filename):
    ...
    pset = create_particle_set( 
      luminosity=1.e50*numpy.array(luminosity) | ( units.s**-1), 
      x=numpy.array(x) | (units.kpc), 
      y=numpy.array(y) | (units.kpc), 
      z=numpy.array(z) | (units.kpc), 
      SpcType=numpy.array(spctype) )
    
    return pset


if __name__=="__main__":
  rad = SPHRay(redirection="none")#channel_type="ibis", hostname="hofvijver")
...
#  gas.u=gas.u*100
  ...
  #rad.src_particles.remove_particles(src)
