...
void advance_and_find_timesteps(void)
{
  ...
  for(i = 0, dispmax = 0, disp2sum = 0; i < NumPart; i++)
      ...
      for(j = 0; j < 3; j++)
	{
	  ...
	  P[i].GravAccel[j] += P[i].FixAccel[j];
	  P[i].FixAccel[j] = 0;
#endif
	}
...
#ifdef FIXEDPOT
	      dv[j] += P[i].FixAccel[j] * dt_gravkick;
#endif
	      P[i].Vel[j] += dv[j];
	    }
...
#ifdef FIXEDPOT
		  SphP[i].VelPred[j] -= P[i].FixAccel[j] * dt_gravkick2;		  
#endif
...
#ifdef SPHS
              /* Update masses if mass diffusion is required (for multimass) */
	      P[i].Mass += SphP[i].MassDiss * dt_entr;
	      
	      /* Update alpha dissipation terms */
	      SphP[i].Alpv += SphP[i].DtAlpv * dt_entr;
...
#ifdef FIXEDPOT
		  SphP[i].VelPred[j] += P[i].FixAccel[j] * dt_gravkickA;  
#endif
		}
	    }
	}
				   ...
				   aphys */ )
{
  ...
  if(P[p].Type == 0)
    {
#ifdef SHRINKSTEP
      maxsigvel = SphP[p].MaxSignalVel;
      if(All.ComovingIntegrationOn)
        dt_courant = 2 * All.CourantFac * All.Time * SphP[p].Hmean / (fac3 * maxsigvel);
      else
        dt_courant = 2 * All.CourantFac * SphP[p].Hmean / maxsigvel;
#elif SHRINKCOURANT
      maxsigvel = SphP[p].MaxSignalVel;
      if(All.ComovingIntegrationOn)
        dt_courant = 2 * SphP[p].CourantMin * All.Time * SphP[p].Hsml / (fac3 * maxsigvel);
      else
        dt_courant = 2 * SphP[p].CourantMin * SphP[p].Hsml / maxsigvel;
#else
      maxsigvel = SphP[p].MaxSignalVel;
      if(All.ComovingIntegrationOn)
        dt_courant = 2 * All.CourantFac * All.Time * SphP[p].Hsml / (fac3 * maxsigvel);
      else
        dt_courant = 2 * All.CourantFac * SphP[p].Hsml / maxsigvel;
#endif
      if(dt_courant < dt)
	dt = dt_courant;
    }
...
#endif
#ifdef FIXEDPOT
      printf("fixpot_force=(%g|%g|%g)\n", P[p].FixAccel[0], P[p].FixAccel[1], P[p].FixAccel[2]);
#endif
      if(P[p].Type == 0)
