...
#define  GADGETVERSION   "2.0-SPHS"   /*!< code version string */
...
#define  MAXTOPNODES     400000   /*!< Maximum number of nodes in the top-level tree used for domain decomposition */
...
#ifdef SPHS
  double ArtBulkDissConst; /*!< Sets the parameter \f$\alpha\f$ for the dissipation switches */
#endif
#ifdef SHRINKCOURANT
  double CourantMin;
#endif
...
int FixedPotential; /*! select which fixed potential to use from fixedpotential.c */
#endif
#ifdef ICSMOOTH
double FixedPressure; /*! set initial pressure (entropy will be smoothed accordingly) */
#endif
#ifdef GLASSIFY
double GlassTol;
#endif
...
int TimeStep;
#endif
#ifdef FIXEDPOT /*!< parameters for adding a fixed potential */
/* JUSTIN'S FIXEDPOT CODE */
FLOAT FixAccel[3];
FLOAT FixPot;
#endif
...
#ifdef SPHS
FLOAT EntDiss; /*!< local entropy dissipation */
FLOAT MassDiss; /*!< local mass dissipation */
FLOAT Alpv; /*!< dissipation parameter */
FLOAT QVx[10];
FLOAT QVy[10];
FLOAT QVz[10];
FLOAT M0[10];
...
#endif
#ifdef ERRORS
FLOAT Error0[3]; /*!< output errors if needed */ 
#endif
}
