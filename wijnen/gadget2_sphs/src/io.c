...
void fill_write_buffer(enum iofields blocknr, int *startindex, int pc, int type)
{
  ...
  switch (blocknr)
...
#ifdef FIXEDPOT
	    for(k = 0; k < 3; k++)
	      fp[k] += P[pindex].FixAccel[k] * dt_gravkick;
#endif
    ...
    case IO_ENTR:               /* Entr. dissipation */
#ifdef SPHSOUT
#ifdef SPHS
      for(n = 0; n < pc; pindex++)
        if(P[pindex].Type == type)
          {
            *fp++ = SphP[pindex].EntDiss;
            n++;
          }
#endif
#endif
    ...
    case IO_MASD:               /* Mass dissipation */
#ifdef SPHSOUT
#ifdef SPHS
      for(n = 0; n < pc; pindex++)
        if(P[pindex].Type == type)
          {
            *fp++ = SphP[pindex].MassDiss;
            n++;
          }
#endif
#endif
    ...
    case IO_APVL:               /* Diss. stuff */
#ifdef SPHSOUT
#ifdef SPHS
      for(n = 0; n < pc; pindex++)
        if(P[pindex].Type == type)
          {
            *fp++ = SphP[pindex].Alpvloc;
            n++;
          }
#endif
#endif
    ...
    case IO_DTAV:               /* Diss. stuff */
#ifdef SPHSOUT
#ifdef SPHS
      for(n = 0; n < pc; pindex++)
        if(P[pindex].Type == type)
          {
            *fp++ = SphP[pindex].DtAlpv;
            n++;
          }
#endif
#endif
    ...
    case IO_ERRORS0x:		/* errors */
#ifdef ERRORS
      for(n = 0; n < pc; pindex++)
	if(P[pindex].Type == type)
	  {
	    *fp++ = SphP[pindex].Error0[0];
	    n++;
	  }
#endif
      break;
    case IO_ERRORS0y:		/* errors */
#ifdef ERRORS
      for(n = 0; n < pc; pindex++)
	if(P[pindex].Type == type)
	  {
	    *fp++ = SphP[pindex].Error0[1];
	    n++;
	  }
#endif
      break;
    case IO_ERRORS0z:		/* errors */
#ifdef ERRORS
      for(n = 0; n < pc; pindex++)
	if(P[pindex].Type == type)
	  {
	    *fp++ = SphP[pindex].Error0[2];
	    n++;
	  }
#endif
      break;
...
int get_bytes_per_blockelement(enum iofields blocknr)
{
  ...
  switch (blocknr)
    ...
    case IO_IQVY:
    case IO_IQVZ:
      bytes_per_blockelement = 10 * sizeof(float);
      break;
    }
...
#endif
#ifndef ERRORS
  if(blocknr == IO_ERRORS0x)
    return 0;
  if(blocknr == IO_ERRORS0y)
    return 0;
  if(blocknr == IO_ERRORS0z)
    return 0;
#endif
#ifndef SPHS
  if(blocknr == IO_RHOTHERM)
    return 0;
#endif
...
void fill_Tab_IO_Labels(void)
{
  ...
  for(i = 0; i < IO_NBLOCKS; i++)
	...
	/* JUSTIN'S CODE */
      case IO_ALPV:
        strncpy(Tab_IO_Labels[IO_ALPV], "ALPV", 4);
        break;
      case IO_ENTR:
        strncpy(Tab_IO_Labels[IO_ENTR], "ENTR", 4);
        break;
      case IO_MASD:
        strncpy(Tab_IO_Labels[IO_MASD], "MASD", 4);
        break;
      case IO_APVL:
        strncpy(Tab_IO_Labels[IO_APVL], "APVL", 4);
        break;
      case IO_DTAV:
        strncpy(Tab_IO_Labels[IO_DTAV], "DTAV", 4);
        break;
      case IO_ERRORS0x:
	strncpy(Tab_IO_Labels[IO_ERRORS0x], "ER0X", 4);
	break;
      case IO_ERRORS0y:
	strncpy(Tab_IO_Labels[IO_ERRORS0y], "ER0Y", 4);
	break;
      case IO_ERRORS0z:
	strncpy(Tab_IO_Labels[IO_ERRORS0z], "ER0Z", 4);
	break;
      case IO_RHOTHERM:
        strncpy(Tab_IO_Labels[IO_RHOTHERM], "RHOT", 4);
        break;
      case IO_IQVX:
        strncpy(Tab_IO_Labels[IO_IQVX], "IQVX", 4);
        break;
      case IO_IQVY:
        strncpy(Tab_IO_Labels[IO_IQVY], "IQVY", 4);
        break;
      case IO_IQVZ:
        strncpy(Tab_IO_Labels[IO_IQVZ], "IQVZ", 4);
        break;
      }
}
