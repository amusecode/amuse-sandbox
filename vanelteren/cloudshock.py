...
class CalculateCloudShock(object):
    ...
    def store_sampled_grid(self, instance, step):
        sample = self.sample_grid(instance)
        
        print "writing results"   
        io.write_set_to_file(
            sample, 
            "cloudshock_{1}_{0}.hdf5".format(self.number_of_grid_points, self.name_of_the_code),
            'amuse',
            apped_to_file = True
        )
        print "done"   
    ...
    def get_solution_at_time(self, time):
        ...
        self.store_sampled_grid(instance, 0)
        ...
        if time > 0.0 | generic_unit_system.time:
            ...
            while t <= time:
                ...
                self.store_sampled_grid(instance, step)
...
def main():
    ...
    result = model.get_solution_at_time(4.0 * model.get_tau())
