    ...
    def evolve(self, start_date, end_date, step):
        ...
        for x in arange(start_date, end_date, step):
            ...
            if not self.code.get_indices_of_colliding_particles()['index_of_particle1'] == -1:
                print "collision detected, removing "
...
if __name__ == '__main__':
    ...
    native_planets_from_code_to_model = I.code.particles.new_channel_to(planets)
    all_particles = planets.copy()

    P = planetarium.SolarSystemView((640,400))

    for t in range(start_date.toordinal(), stop_date.toordinal(), 1):
        
        if t==voyagerI_launch_date.toordinal():
            print "Voyager Launch, insterting particle.."
            voyagerI = core.Stars(1)
            set_IC_at_date([planets_data.VoyagerI],voyagerI, voyagerI_launch_date)
        ...
        native_planets_from_code_to_model.copy()
