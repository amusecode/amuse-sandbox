...
# (note the systems share the same coordinate frame, although units may differ)    
    ...
    shift_sys(cluster,Rinit,0| units.parsec,0.|units.parsec,
                  0| units.kms,vcirc,0| units.kms,omega)
...
# evolve and make plots
    ...
    for i,t in enumerate(times):
        ...
        pyplot.plot(x,y,'r .')
        pyplot.plot([0.],[0.],'b +')
        pyplot.xlim(xcm.value_in(units.parsec)-20,xcm.value_in(units.parsec)+20)
        pyplot.ylim(ycm.value_in(units.parsec)-20,ycm.value_in(units.parsec)+20)
